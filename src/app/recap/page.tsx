...
import { generateRecap, getLatestRecap, type RecapData } from "@/lib/recap-actions";
...
export default function RecapPage() {
    ...
    useEffect(() => {
        // Check for existing recap on mount
        getLatestRecap().then((res) => {
            if (res?.data) {
                setData(res.data);
            }
            setLoading(false);
    ...
    const handleGenerate = () => {
        startTransition(async () => {
            const newData = await generateRecap();
            setData(newData);
        });
    };
